...
public class DrugControllerIntegrationTest {
	...
	public void allTestsToVerifyFlow() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.fileUpload("/drug/saveDrug").file(fileWithContent)
					...
					.param("drugName", "testDrugName")
					.param("strength", "testStrength")
					.param("dosageForm", "testDosageForm")
					.param("storage", "testStorage")
					.param("packSize", "testSize")
					.param("drugprice", "50.56")
	   ...
	   // Test 3 new drug with empty file after insert count will be 2
		...
		this.mockMvc.perform(MockMvcRequestBuilders.fileUpload("/drug/saveDrug").file(emptyFile)
				...
				.param("genericId", "1")
				.param("manufacturerId", "1")
				.param("drugName", "testDrugName2")
				.param("strength", "testStrength2")
				.param("dosageForm", "testDosageForm2")
				.param("storage", "testStorage2")
				.param("packSize", "testSize")
				.param("drugprice", "50.56")
     ...
     // Test 4 After delete count should be 1
		...
		mockMvc.perform(get("/drug/1")).
		andExpect(status().isOk()).
		andExpect(view().name("drugs/drugshow")).
		andExpect(content().string(Matchers.containsString("testDrugName")));
		
		
		//Test 5 when a item edited it should be reflected on show drug information and count should remain 1
		
		this.mockMvc.perform(MockMvcRequestBuilders.fileUpload("/drug/saveDrug").file(fileWithContent)
			    .param("id","1")
				.param("genericId", "1")
				.param("manufacturerId", "1")
				.param("drugName", "testDrugNameChanged")
				.param("strength", "testStrength")
				.param("dosageForm", "testDosageForm")
				.param("storage", "testStorage")
				.param("packSize", "testSize")
		...
		andExpect(status().isOk()).
		andExpect(view().name("drugs/drugshow")).
		andExpect(content().string(Matchers.containsString("testDrugNameChanged")));
